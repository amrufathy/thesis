...
from src.models.modules import Expander, Compressor
...
class CycleArchitecture(nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        self.expander = Expander(tokenizer=self.tokenizer)
        self.compressor = Compressor(tokenizer=self.tokenizer)
    ...
    def forward(self, dict_input: Dict) -> Dict:
        ...
        expansion_loss, expansion_logits = expansion_results['loss'], expansion_results['logits']
        expansion_accuracy, expansion_bleu = expansion_results['accuracy'], expansion_results['bleu']

        # overwrite dict_input['story_ids'] (original story ids) with generated_story_ids
        generated_story_ids = argmax(expansion_logits, dim=-1) # TODO: use gumbel softmax
        ...
        compression_accuracy, compression_bleu = compression_results['accuracy'], compression_results['bleu']

        # INFO - Step 3: Calculate Reconstruction Loss
        # ignore <s> and reshape
        reconstructed_logits = compression_logits[:, 1:].contiguous().view(-1, compression_logits.size(-1))
